...
import scanpy as sc
import pandas as pd
input_path = args.datasetDir+args.datasetName+'.csv'
X_seq = pd.read_csv(input_path,index_col=0)
scData = scDataset(X_seq.values)
#scData = scDataset(data)

genelist = list(range(X_seq.shape[0]))
celllist = list(range(X_seq.shape[1]))
