...
#Imputation-Drop Rate
parser.add_argument('--my_drop_rate', type=float, default=0.1)
...
import scanpy as sc
input_h5ad_path = args.datasetDir+args.datasetName
aData = sc.read_h5ad(input_h5ad_path)
aData.X[np.isnan(aData.X)] = 0

X_all = aData.X
final_mask, final_x = DropData(X_all, d_rate=args.my_drop_rate)

genelist = list(range(aData.X.shape[1]))
celllist = list(range(aData.X.shape[0]))

X_df = pd.DataFrame(X_all)
X_df.to_csv(args.outputDir+args.datasetName+'_True.csv')

mask_df = pd.DataFrame(final_mask)
mask_df.to_csv(args.outputDir+args.datasetName+'_'+str(args.my_drop_rate*10)+'_Mask.csv')

aData.X = final_x 

scData = scDataset(aData.X.T)
#scData = scDataset(data)
...
if __name__ == "__main__":
    ...
    recon_df.to_csv(args.outputDir+args.datasetName+'_Imp_'+str(args.my_drop_rate*10)+'_recon.csv')
    emblist = []
    for i in range(zOut.shape[1]):
        emblist.append('embedding'+str(i))
    embedding_df = pd.DataFrame(zOut, index=celllist, columns=emblist)
